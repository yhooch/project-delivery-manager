...
- 列表控制条：搜索框（输入防抖）、排序（最近编辑 / 最近创建 / 标题）、显示密度切换（舒适 / 紧凑，本地记忆偏好）；
  快速筛选按当前落地固定为全部、我创建、已归档。模型生成和通过模型修改不作为首页快速筛选入口，
  通过列表来源图标、创建信息和详情页创建/最近编辑来源展示。
...
- 如正文中存在一级标题，则默认提取第一个一级标题为标题。
- 否则标题使用文件名或用户输入标题。
- `contentMarkdown` 保存原始 Markdown。
- `contentText` 从 Markdown 中提取纯文本。
- 正文图片第一版只允许受控引用；远程图片保留为文本链接或占位，避免绕过附件权限。
- 上传文件需校验扩展名、MIME 和大小；Markdown 按 UTF-8 解码，不额外做非法字符编码显式拒绝。超出限制时拒绝导入，不创建半成品文档。
...
- 标签筛选。
- 关联资源筛选。
- 来源和最近编辑来源展示：用户导入、用户粘贴、模型创建、模型修改。
  当前首页不提供“模型生成 / 通过模型修改”快速筛选入口，以实际落地为准。
- 状态筛选：活跃、已归档。
...
- 文档列表返回轻量 `DocumentListItem`，只包含列表必需元数据、标签、关联资源和文件夹路径；不返回 `contentMarkdown`、`contentText`、chunks、评论、附件或时间线详情。
- 列表查询不得为展示摘要拉取全文大字段；如后续列表搜索需要摘要，只允许后端在当前分页结果内生成有长度上限的 `contentSnippet`，并优先评估 chunk / FTS 命中能力。
...
不返回维护型摘要字段，也不返回 `contentMarkdown` / `contentText` 全文。当前落地为轻量文档项 + 每篇文档最多 3 条当前 revision 的 chunk 命中；chunk 命中查询只针对当前分页结果中的文档。

`document.get` 支持：

- 返回全文 Markdown。
- 返回 chunks。
- 返回关联资源、标签、评论概览、附件概览和时间线概览；概览 items 只取有限首批，同时返回数据库 count 得到的真实 total，右侧栏计数使用 total 而不是首批 items 数量。
...
- 搜索返回 chunk 命中片段，不返回维护型摘要或正文全文字段。
...
> 状态：反向集成暂缓。当前本轮落地以“文档详情页展示关联资源并支持跳转”为准；
> 需求、任务、Bug、事项、版本详情页中的“关联文档入口”和“从资源进入相关文档列表”
> 后置，不纳入当前验收。

目标：

- 在文档详情展示关联资源。
- 后续在需求、任务、Bug、事项、版本详情中展示关联文档入口。
- 后续支持从资源进入相关文档列表。

重点：

- 当前第一版复用现有详情页和详情抽屉，不强制重构为统一资源路由。
- 当前保留后端按资源反查文档的基础契约，前端业务资源详情页入口暂缓。
- 如新增统一资源页，必须先单独确认范围。
