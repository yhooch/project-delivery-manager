...
describe("RealtimeReplayBufferService", () => {
  it("returns events after the Last-Event-ID sequence cursor", () => {
    ...
    const replay = buffer.replayAfter("1", REPLAY_TEST_NOW);
    ...
    expect(buffer.replayAfter("3", REPLAY_TEST_NOW)).toEqual({
      currentSequence: 3,
      events: [],
      ok: true,
    });
  ...
  it("returns REPLAY_EXPIRED when TTL removed a required event", () => {
    ...
    expect(buffer.replayAfter("1", now)).toEqual({
      currentSequence: 3,
      ok: false,
  ...
  it("returns REPLAY_EXPIRED when TTL removed the whole replay cache", () => {
    ...
    expect(buffer.replayAfter("1", replayAt)).toEqual({
      currentSequence: 2,
      ok: false,
      reason: "REPLAY_EXPIRED",
    });
  ...
  it("returns SERVER_RESTART for cursors beyond current sequence or an empty cache with a cursor", () => {
    ...
    expect(buffer.replayAfter("2", REPLAY_TEST_NOW)).toEqual({
      currentSequence: 1,
      ok: false,
      reason: "SERVER_RESTART",
    });
